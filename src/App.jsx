...
import { MapContainer, TileLayer, CircleMarker, Marker, Polyline, Polygon, ImageOverlay, WMSTileLayer } from 'react-leaflet';
    ...
    <div className="relative h-[100dvh] w-full overflow-hidden bg-[#0f172a] font-sans select-none text-slate-100 flex flex-col">
      ...
      <div className={`absolute inset-0 transition-all duration-700 z-0 ${activeTab !== 'explore' ? 'blur-md scale-105 opacity-40 pointer-events-none' : 'opacity-100'}`}>
        ...
        >
          ...
          {layers.filter(layer => layer.active && layer.serviceType === 'wms' && layer.url && layer.layers).map(layer => (
            <WMSTileLayer
              key={`wms-${layer.id}`}
              url={layer.url}
              layers={layer.layers}
              ...
              attribution={layer.attribution || layer.name}
            />
          ))}

          {layers.filter(layer => layer.active && layer.serviceType === 'raster' && layer.url && layer.bounds).map(layer => (
            <ImageOverlay
              key={`raster-${layer.id}`}
              url={layer.url}
              bounds={layer.bounds}
