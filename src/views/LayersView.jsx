...
const COLOR_OPTIONS = [
  ...
  { label: 'Violet', hex: '#8b5cf6', tw: 'bg-violet-500' },
  { label: 'Orange', hex: '#f97316', tw: 'bg-orange-500' },
  { label: 'Rose', hex: '#f43f5e', tw: 'bg-rose-500' },
  { label: 'Yellow', hex: '#eab308', tw: 'bg-yellow-500' },
  { label: 'Cyan', hex: '#06b6d4', tw: 'bg-cyan-500' },
  { label: 'Slate', hex: '#64748b', tw: 'bg-slate-500' },
];

function SymbologyPanel({ layer, onUpdate, onClose }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="px-6 py-5 border-b border-white/10 flex justify-between items-center">
          <div>
            <h3 className="text-sm font-bold text-white uppercase tracking-widest">Symbology</h3>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
          ...
          <div className="space-y-3">
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Rendering Mode</p>
            <div className="flex bg-black/30 p-1 rounded-xl gap-1">
              {[
                { id: 'single', label: 'Single Color' },
                { id: 'categorized', label: 'By Attribute' }
              ].map(m => (
                <button 
                  key={m.id}
          ...
          {mode === 'single' && (
            <div className="space-y-3">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Layer Color</p>
              ...
              <div className="flex items-center gap-3 p-3 rounded-xl bg-black/20 border border-white/5">
                ...
                <span className="text-[10px] text-slate-600 ml-auto">Custom</span>
              </div>
            </div>
          )}
          ...
          {mode === 'categorized' && (
            <div className="space-y-4">
              <div className="space-y-2">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Attribute</p>
                {attributes.length === 0 ? (
                  <p className="text-[10px] text-slate-600 italic p-3 rounded-xl bg-black/20 border border-white/5">
                    No attributes defined. Add fields in the layer schema first.
                  </p>
                ) : (
                  <select
                    value={selectedAttr}
              ...
              {selectedAttr && (
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Color Rules</p>
                    <button
                      onClick={addRule}
                  ...
                  {rules.length === 0 && (
                    <p className="text-[10px] text-slate-600 italic p-3 rounded-xl bg-black/20 border border-white/5">
                      Click "Add Rule" to define a color for a specific attribute value.
                    </p>
                  )}
                  ...
                  {rules.length > 0 && (
                    <div className="p-3 rounded-xl bg-primary/5 border border-primary/20">
                      <p className="text-[9px] text-primary/60 uppercase tracking-wider font-bold">
                        All other values → rendered with layer base color
                      </p>
                    </div>
                  )}
                </div>
        ...
        <div className="px-6 py-4 border-t border-white/10 bg-black/20 flex justify-end gap-3">
          <button onClick={onClose} className="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-widest hover:text-white transition-colors">Cancel</button>
...
export default function LayersView({ 
  ...
  const [symbologyLayerId, setSymbologyLayerId] = useState(null);
  const symbologyLayer = symbologyLayerId ? layers.find(l => l.id === symbologyLayerId) : null;

  const updateLayerSymbology = (id, updates) => {
    ...
    <div className="w-full max-w-4xl h-full mx-auto flex flex-col items-center animate-in fade-in duration-500 pointer-events-auto">
      {symbologyLayer && (
        <SymbologyPanel 
          ...
          onUpdate={(updates) => updateLayerSymbology(symbologyLayer.id, updates)}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-8">
          {layers.filter(l => l.name.toLowerCase().includes(layerFilter.toLowerCase())).length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center gap-4 text-slate-600 py-16">
              ...
              <p className="text-sm font-bold uppercase tracking-widest">{layerFilter ? 'No results' : 'No layers yet'}</p>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                    {/* Symbology button */}
                    <button
                      onClick={(e) => { e.stopPropagation(); setSymbologyLayerId(layer.id); }}
                ...
                {layer.symbology?.mode === 'categorized' && layer.symbology?.attribute && (
                  <div className="flex items-center gap-2 px-2">
                    ...
                    <span className="text-[9px] text-primary/60 font-bold uppercase tracking-wider">By: {layer.symbology.attribute}</span>
