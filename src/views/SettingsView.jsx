...
export default function ImpostazioniView({ draftSettings, draftImpostazioni, setDraftSettings, saveSettings, showTutorialAgain, language = 'it' }) {
  ...
  const addCustomCrs = () => {
    ...
    if (!ok) {
      setCrsMessage('CRS personalizzato non valido. Usa una stringa PROJ, ad esempio +proj=utm +zone=32 +datum=WGS84 +units=m +no_defs.');
      return;
    }
    setDraftSettings(prev => ({ ...prev, projectCrs: code, crsOverride: true }));
    ...
    <div className="w-full max-w-4xl h-full flex flex-col items-center animate-in fade-in duration-500 pointer-events-auto">
      ...
      <div className="flex-1 w-full glass rounded-[2rem] sm:rounded-[2.5rem] border border-white/10 overflow-hidden flex flex-col min-h-0">
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 sm:p-8 space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="p-5 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-between cursor-pointer hover:bg-white/10 transition-colors" onClick={() => set('crsOverride', !s.crsOverride)}>
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-1">Usa il CRS progetto per coordinate, misure ed esportazioni quando è trasformabile.</p>
                </div>
          ...
          <section>
            <SectionHeader icon={<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />} title={language === 'en' ? 'Display' : 'Visualizzazione'} />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2"><p className="text-[10px] font-bold text-slate-500 uppercase">{tt('theme')}</p><SegmentControl value={s.theme} onChange={v => set('theme', v)} options={[{ value: 'light', label: tt('light') }, { value: 'dark', label: tt('dark') }]} /><p className="text-[9px] text-slate-600">L'anteprima del tema è immediata; Salva modifiche la rende permanente.</p></div>
              <div className="space-y-2"><p className="text-[10px] font-bold text-slate-500 uppercase">{tt('units')}</p><SegmentControl value={s.units} onChange={v => set('units', v)} options={[{ value: 'metric', label: `${tt('metric')} (m, km)` }, { value: 'imperial', label: `${tt('imperial')} (ft, mi)` }]} /><p className="text-[9px] text-slate-600">Influisce su scala, misure e visualizzazione coordinate.</p></div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="p-5 rounded-2xl bg-white/5 border border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><p className="text-xs font-bold text-white uppercase">{tt('quickGuide')}</p><p className="text-[10px] text-slate-500 mt-1">Mostra di nuovo il tutorial iniziale.</p></div><button onClick={showTutorialAgain} className="px-5 py-3 rounded-xl bg-primary/20 border border-primary/30 text-primary text-[10px] font-bold uppercase tracking-widest hover:bg-primary hover:text-white transition-all">{tt('showTutorial')}</button></div>
          </section>
        </div>
