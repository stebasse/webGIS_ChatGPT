...
export default function ExploreHUD({
  ...
  return (
    <div className="w-full h-full flex flex-col animate-in fade-in duration-500 pointer-events-none">
      ...
      {showGoTo && (
        <div className="absolute top-[calc(5.9rem+env(safe-area-inset-top,0px))] left-4 sm:left-6 pointer-events-auto glass p-4 rounded-2xl border border-white/15 shadow-2xl w-[min(92vw,320px)] space-y-3">
      ...
      <div className="absolute top-[calc(0.45rem+env(safe-area-inset-top,0px))] left-4 right-4 sm:left-6 sm:right-6 flex items-start justify-between gap-3 pointer-events-none z-30">
        <div className="min-w-0 max-w-[48vw] sm:max-w-xs">
          {activeLayer ? (
            <div className="glass px-3 py-1.5 rounded-2xl border border-white/15 flex items-center gap-2 shadow-xl">
              <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: activeLayer.colorHex || '#0ea5e9' }} />
              <div className="min-w-0">
                <div className="text-[7px] text-slate-500 uppercase tracking-widest leading-none">Active layer</div>
                <div className="text-[9px] font-bold text-white uppercase tracking-widest truncate">{activeLayer.name}</div>
              </div>
            </div>
          ) : (
            <div className="glass px-3 py-1.5 rounded-2xl border border-amber-500/30 bg-amber-500/10 flex items-center gap-2 shadow-xl">
        ...
        {projectCrs && (
          <div className="glass px-3 py-1.5 rounded-2xl border border-white/10 text-right shadow-xl max-w-[42vw] sm:max-w-xs">
            <div className="text-[7px] text-slate-500 uppercase tracking-widest leading-none">Project CRS</div>
            <div className="text-[9px] font-mono font-bold text-primary truncate">{projectCrs}</div>
          </div>
        )}
      </div>
